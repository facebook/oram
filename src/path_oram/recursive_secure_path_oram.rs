...
mod block_oram_tests {
    ...
    #[test]
    #[ignore]
    fn debug_large_capacity() {
        init_logger();

        let mut rng = StdRng::seed_from_u64(0);
        let mut oram = ConcreteObliviousBlockOram::<4096, BlockValue<4096>>::new(4096, &mut rng);
        dbg!(&oram.read(0, &mut rng));
    }

    #[test]
    #[ignore]
    fn trace_normal_test_init() {
        init_logger();

        let mut rng = StdRng::seed_from_u64(0);
        let mut _oram = ConcreteObliviousBlockOram::<64, BlockValue<4096>>::new(4096, &mut rng);
    }
